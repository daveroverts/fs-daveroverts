...
const BlogPost = ({ post }: { post: Post }) => {
  return (
    <Link href={`/posts/${post.slug}`} className="hover:underline">
      <article
        className={`overflow-hidden group flex flex-col justify-center min-h-full text-center rounded-3xl ${
          !post.banner ? "border-2 hover:bg-gray-50 dark:hover:bg-gray-800" : ""
        }`}
      >
